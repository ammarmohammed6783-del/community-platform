      ...
      <div className="flex items-center justify-between relative">
            ...
            post?.author?.profilePhoto ? (
              ...
              <div className="relative">
                <div className="w-11 h-11 rounded-2xl bg-linear-to-tr from-indigo-600 via-indigo-500 to-purple-500 text-white font-bold text-sm flex items-center justify-center shadow-md shadow-indigo-500/20">
                  {post.author.name?.[0]?.toUpperCase() || "unKnown"}
                </div>
        ...
        {!isOwnPost && (
          <div ref={menuRef} className="relative">
            ...
            {open && (
              <ul className="absolute top-6 right-0 z-10 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg py-2 w-40">
                <li
                  className="px-4 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-700 cursor-pointer"
                  onClick={followHim}
                >
      ...
      <div className="pt-3 border-t border-slate-200/70 dark:border-slate-800/70 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
        <div className="flex items-center gap-4">
          ...
          <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-200 transition-all cursor-pointer">
            ...
            <Link href={`/feed/${post.id}`}>Comment</Link>
          </button>
        </div>
